...
function matchesKnownSecretValue(value: string, additionalPatterns: SecretPatternDefinition[] = []): boolean {
...
function isLikelySensitiveStandaloneValue(rawValue: string, additionalPatterns: SecretPatternDefinition[] = []): boolean {
	...
	if (matchesKnownSecretValue(value, additionalPatterns)) {
		return true;
	}
...
function shouldRedactSensitiveKeyValue(
	...
	if (isStandaloneAuthCredentialKey(key)) {
		return isLikelySensitiveStandaloneValue(rawValue, additionalPatterns);
	}
...
function shouldRedactJsonSensitiveKeyValue(
	...
	if (isStandaloneAuthCredentialKey(key)) {
		return typeof value === "string" && isLikelySensitiveStandaloneValue(value, additionalPatterns);
	}
...
function redactJsonValue(
	...
	if (Array.isArray(value)) {
		return value.map((entry) => redactJsonValue(entry, placeholder, keyPatterns, state, additionalPatterns));
	}
	...
	for (const [key, entry] of Object.entries(value)) {
		if (shouldRedactJsonSensitiveKeyValue(key, entry, placeholder, keyPatterns, additionalPatterns)) {
			redactedObject[key] = placeholder;
			state.redactionCount += 1;
			continue;
		}

		redactedObject[key] = redactJsonValue(entry, placeholder, keyPatterns, state, additionalPatterns);
	}
...
function redactJsonStructuredContent(
	...
	try {
		...
		const redacted = redactJsonValue(parsed, placeholder, keyPatterns, state, additionalPatterns);
...
function redactStructuredLine(
	...
	if (jsonMatch) {
		...
		if (
			key &&
			value !== undefined &&
			shouldRedactSensitiveKeyValue(key, value, placeholder, keyPatterns, additionalPatterns)
		) {
			return {
...
function redactStructuredValues(
	...
	const redactedLines = lines.map((part) => {
		...
		const redacted = redactStructuredLine(part, placeholder, keyPatterns, additionalPatterns);
...
export function redactSensitiveReadContent(
	...
	const secretPatternResult = config.redactSecretPatterns
		? redactKnownSecretPatterns(embedded.content, config.placeholder, customPatterns)
